/**
 * useTheme.ts 单测 —— 主题切换 API
 *
 * 注：useTheme 现在内部定义的 themes / themeAnimations / themeTransitioning
 * / themeFlash 常量都来自自身文件（不再走 vanilla-extract），所以测试不需要
 * vi.mock utility.css。
 */
...
describe('setThemeSync(name)', () => {
  it('切到 dark 时挂 themes.dark + .dark', () => {
    ...
    expect(html.classList.contains(themes.dark)).toBe(true)
    expect(html.classList.contains('dark')).toBe(true)
  })

  it('切到 light 时移除 themes.dark / .dark', () => {
    document.documentElement.classList.add(themes.dark, 'dark')
    setThemeSync('light')
  ...
  it('切到 brand / accent 时挂对应 className，不挂 .dark', () => {
    setThemeSync('brand')
    expect(document.documentElement.classList.contains(themes.brand)).toBe(true)
    expect(document.documentElement.classList.contains('dark')).toBe(false)

    setThemeSync('accent')
    expect(document.documentElement.classList.contains(themes.accent)).toBe(true)
    expect(document.documentElement.classList.contains('dark')).toBe(false)
  })
...
describe('beginThemeTransition(opts)', () => {
  it('挂 themeTransitioning className', () => {
    ...
    expect(document.documentElement.classList.contains(themeTransitioning)).toBe(true)
  })
  ...
  it('duration 后清理 themeTransitioning（默认 350ms）', () => {
    ...
    expect(document.documentElement.classList.contains(themeTransitioning)).toBe(false)
  })
...
describe('triggerThemeFlash()', () => {
  it('挂 themeFlash 到 body', () => {
    triggerThemeFlash()
    expect(document.body.classList.contains(themeFlash)).toBe(true)
  })
  ...
  it('duration 后清理（默认 200ms）', () => {
    ...
    expect(document.body.classList.contains(themeFlash)).toBe(false)
  })
})
...
describe('setTheme(name, opts)', () => {
  ...
  it('animation: "fade" 默认挂 themeAnimations.fade', () => {
    void setTheme('brand')
    expect(document.body.classList.contains(themeAnimations.fade)).toBe(true)
  })

  it('animation: "blur" 挂 themeAnimations.blur', () => {
    void setTheme('brand', { animation: 'blur' })
    expect(document.body.classList.contains(themeAnimations.blur)).toBe(true)
  })

  it('animation: "shimmer" 挂 themeAnimations.shimmer', () => {
    void setTheme('accent', { animation: 'shimmer' })
    expect(document.body.classList.contains(themeAnimations.shimmer)).toBe(true)
  })

  it('animation: false 不挂任何 body 动画', () => {
    void setTheme('brand', { animation: false })
    for (const key of Object.keys(themeAnimations)) {
      expect(
        document.body.classList.contains(
          themeAnimations[key as keyof typeof themeAnimations],
        ),
      ).toBe(false)
    }
  })
...
describe('ThemeName 类型完整性', () => {
  it('4 套主题都能作为 ThemeName 接受（运行时）', () => {
    const cases = [
      { name: 'light' as const, themeClass: themes.light },
      { name: 'dark' as const, themeClass: themes.dark },
      { name: 'brand' as const, themeClass: themes.brand },
      { name: 'accent' as const, themeClass: themes.accent },
    ]
    for (const { name, themeClass } of cases) {
      // 清理前序状态
      document.documentElement.className = ''
      setThemeSync(name)
      // light 是默认主题，setThemeSync 的设计：light 不挂 themes.light className
      if (name === 'light') {
